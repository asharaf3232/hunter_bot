# ----------------------------------------------------------------------------------
# # 💎 بوت صياد الجواهر - النسخة النهائية (المصححة) 💎
# ----------------------------------------------------------------------------------
#
# الإصدار: 10.0 (الإنتاج المصحح)
#
# التصحيحات الرئيسية:
#   1.  **إصلاح المصادقة:** تم إصلاح دالة `analyze_contract_with_goplus` لتقوم بإرسال
#       مفتاح GoPlus API في هيدر "Authorization" بشكل صحيح.
#   2.  **تحسين الأداء:** تم استبدال مكتبة `requests` المتزامنة بمكتبة `aiohttp`
#       غير المتزامنة لإجراء الطلبات، مما يمنع تعطل البوت ويحسن استجابته.
...
async def analyze_contract_with_goplus(contract_address):
    # **التصحيح الرئيسي هنا: استخدام aiohttp وإضافة هيدر المصادقة**
    url = f"https://api.gopluslabs.io/api/v1/token_security/56?contract_addresses={contract_address}"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {GOPLUS_API_KEY}" # استخدام المفتاح
    }
    try:
            ...
            async with session.get(url, headers=headers, timeout=15) as response:
                ...
                if social_profiles_div:
                    ...
                    for link in links:
                        ...
                        elif 'coingecko' in href: socials['coingecko'] = href
                        else:
                            socials.setdefault('website', href)
    except Exception as e:
...
def generate_recommendation(analysis_data, social_data, telegram_subs):
    ...
    if analysis_data.get('is_honeypot') == '0':
        score += 2
        strengths.append("✅ *عقد آمن:* تم التأكد أنه ليس فخًا (Honeypot).")
    else:
        score -= 5
        risks.append("🚨 *خطر فادح:* العقد هو فخ (Honeypot)!")

    buy_tax = float(analysis_data.get('buy_tax', '101')) * 100
    sell_tax = float(analysis_data.get('sell_tax', '101')) * 100
    if buy_tax < 5 and sell_tax < 5:
        score += 1
        strengths.append(f"✅ *ضرائب مقبولة:* الشراء: `{buy_tax:.1f}%` | البيع: `{sell_tax:.1f}%`.")
    else:
        risks.append(f"⚠️ *ضرائب مرتفعة:* الشراء: `{buy_tax:.1f}%` | البيع: `{sell_tax:.1f}%`.")

    if analysis_data.get('owner_address') == '' or analysis_data.get('owner_address') == '0x0000000000000000000000000000000000000000':
        score += 1
        strengths.append("✅ *ملكية متخلى عنها:* لا يمكن للمطور تغيير العقد.")

    if social_data.get('website') and social_data.get('telegram'):
        score += 1
        strengths.append("✅ *وجود أساسي:* تم العثور على موقع وتليجرام.")
    else:
        risks.append("⚠️ *غياب الروابط:* لم يتم العثور على موقع أو تليجرام.")

    if telegram_subs:
        if telegram_subs > 1000:
            score += 1
            strengths.append(f"📈 *مجتمع جيد:* يمتلك `{telegram_subs}` عضو.")
        else:
            risks.append(f"📉 *مجتمع ناشئ:* عدد الأعضاء صغير (`{telegram_subs}` عضو).")

    if score >= 8:
        decision = "💡 *القرار المقترح: فرصة واعدة.* يستحق دراسة أعمق."
    elif 5 <= score < 8:
        decision = "💡 *القرار المقترح: إضافة لقائمة المراقبة.*"
    else:
        decision = "💡 *القرار المقترح: تجنب حاليًا.* يحمل مخاطر عالية."
        
    final_score = f"{max(0, min(10, score))}/10"
    return {"score": final_score, "strengths": strengths, "risks": risks, "decision": decision}

def format_recommendation_report(token_name, recommendation, analysis_data, social_data, telegram_subs):
    ...
    links_section += f"[فحص العقد على BscScan](https://bscscan.com/token/{analysis_data.get('contract_address')})\n"
...
# --- دوال بوت التحكم (دون تغيير) ---
async def start_command(update: Update, context: CallbackContext):
    await update.message.reply_text("أهلاً بك في بوت صياد الجواهر (النسخة المصححة).", reply_markup=get_main_keyboard(context))
...
async def button_callback(update: Update, context: CallbackContext):
    ...
    if action == 'toggle_blockchain':
        ...
        if current_jobs:
            ...
            await query.edit_message_text(text="تم إيقاف مراقبة البلوك تشين.", reply_markup=get_main_keyboard(context))
        else:
            context.job_queue.run_repeating(blockchain_monitoring_job, interval=60, first=1, name=job_name)
            await query.edit_message_text(text="تم بدء مراقبة البلوك تشين.", reply_markup=get_main_keyboard(context))
        ...
        for block_num in range(last_checked_block + 1, latest_block_number + 1):
            ...
            for tx in block.transactions:
                if tx.to is None:
                    ...
                    report = format_recommendation_report(token_name, recommendation, analysis_data, social_data, telegram_subs)

                    await context.bot.send_message(
                        chat_id=int(TELEGRAM_CHAT_ID),
                        text=report, parse_mode=ParseMode.MARKDOWN_V2, disable_web_page_preview=True
                    )
        context.bot_data['last_checked_block'] = latest_block_number
    except Exception as e:
        logger.error(f"Error in blockchain monitoring job: {e}")

# --- وحدة مراقب الإعلانات (دون تغيير) ---
async def news_monitoring_client(bot):
    ...
    async def handler(event):
        ...
        if any(keyword in message_text for keyword in LISTING_KEYWORDS):
            logger.info(f"🚨🚨🚨 Potential listing announcement found in {getattr(event.chat, 'username', 'Unknown')}!")
...
# --- الدالة الرئيسية للتشغيل (دون تغيير) ---
async def main():
    ...
    logger.info("🚀 Gem Hunter Control Bot (Corrected Version) is running...")
